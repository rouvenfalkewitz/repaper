...
| `logo/` | The logo — generated from the font by `build.py`, masters in `logo/svg/`, every icon/social/print/hardware file in `logo/export/` (see `logo/README.md`). |

## The system in ten lines

1. **Name & logo:** RePaper — one word, capital R and P in prose. The logo is the light ring around RE, followed by PAPER in Archivo wide caps: green ring, white/ink letters. `repaper` in code.
...
4. **Accent:** Re Green. `#1EE3A5` "Glow" on dark (the ring, fills, text, LED, merch); `#14C48E` fills/ring and `#0B7259` text on light. Nothing else is ever a brand colour — no orange, no purple.
5. **Primary button = Re Green fill + Carbon/Ink text.** On carbon it's the only element that glows. **The logo never glows** — flat colour everywhere. Glow is light, never ink: buttons on screen and real light sources only, never type, never print.
6. **Functional:** Label Red, Amber, Signal Blue — dark/light variants in the tokens; they only ever mean their state.
7. **Type:** Archivo (logo at width 125 caps, headings at width 100), Figtree body/UI, JetBrains Mono for IDs. 4 px grid, base 16 px.
