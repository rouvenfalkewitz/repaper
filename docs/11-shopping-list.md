...
| 6 | **PN532 NFC module V3** (Elechouse-type, I²C/SPI switchable) — *sold out at BerryBase; get 2× RC522 (MFRC522, SPI, ~2–3 cm range) today and a PN532 clone or Waveshare PN532 HAT from Amazon for the enclosure* | 2 | 8–12 each | [AZ-Delivery](https://www.az-delivery.de) · BerryBase · Amazon | One per Pi. Reads the sheet smytickers                                                                     |
...
**The app is ready and waiting (6 Sep 2026):** RePaper Go builds from `go/` (`./gradlew :app:assembleDebug`; see `docs/12-go-android.md`). Day-one script when the phone arrives: enable developer mode → `adb install go/app/build/outputs/apk/debug/app-debug.apk` → open the app, grant Bluetooth → add Label 2 by its QR link → test page over the phone's own BLE → Settings › Printing: enable "RePaper Go" → print a photo from Google Photos via the system print dialog → claim the phone in the fleet (it joins as kind "go"). The protocol layer is fixture-proven; the day tests the GATT glue and print-dialog flow.
