...
2. **OpenDisplay — the friend's project (https://opendisplay.org)** ← prototype choice
   ...
   - **Field notes from the first label (SoluM 2.1" BWR, nRF52811, firmware 1.0.0, 30 Aug 2026):** (a) firmware 1.x direct-write accepts only one bit plane on BWR/BWY tags (documented in py-opendisplay FINDINGS C1) — the Dock sends such sheets as mono with red drawn black until firmware parity; (b) the tag drops the BLE link during the panel refresh and never sends REFRESH_COMPLETE — the Dock treats "refresh started, then link dropped" as printed; (c) **a refresh on a weak cell browns the tag out**: 2.67 V before → half-cleared panel, reset, 2.25 V after. The Dock reads the battery from the advertisement and refuses to print below 2.7 V. Questions for the friend: BWR two-plane support, link drop during refresh by design?, recommended low-battery cut-off.
