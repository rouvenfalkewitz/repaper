...
class Dock:
    ...
    def run(self):
        ...
        while True:
            ...
            self.state = "job-waiting"; page_no = job.next_page()
            log.info("job %s (%s) page %d/%d waiting — hold a sheet", job.id, job.name, page_no, job.pages)
    ...
    def print_page(self, job: Job, page_no: int, sheet_id: str):
        ...
        self.state = "printing"; job.state = "printing"; job.save()
        src = Image.open(job.page_path(page_no))
        page = render_for_sheet(src, transport.describe(ref))
        t0 = time.time(); res = transport.print(ref, page)
        if res.ok:
    ...
    def snapshot(self) -> dict:
        return {"state": self.state, "message": self.message, "printer": self.cfg["printer_name"],
